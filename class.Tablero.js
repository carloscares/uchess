class Tablero {
	...
	constructor(unaPartida) {
		this.colorCasillaClara = "#eed09d";
		this.colorCasillaOscura = "#804000";
	...
	ponePiezasIniciales() {
		...
		var obj = new Torre(BLANCA, this.casilla['c6']);
		this.casilla['c6'].ocupar(obj);
		...
		obj = new Dama(BLANCA, this.casilla['e6']);
		...
		obj = new Dama(NEGRA, this.casilla['c5']);
		...
		obj = new Torre(NEGRA, this.casilla['h5']);
		this.casilla['h5'].ocupar(obj);
	...
	despliega_tabla_html() {
		...
		for (var i = 8; i > 0; i--) {
			...
			for (var j = 0; j < 8; j++) {
				...
				document.write('<td id="'+letra[j]+i+'" height="40" style="background-color:' + col + ';font-size:35px;">');
				idx = letra[j] + i;
